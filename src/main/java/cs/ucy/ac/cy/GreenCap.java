...
public class GreenCap {
    ...
    private Chromosome firstSelected;
    private Chromosome secondSelected;
    private Chromosome best;
    private int generationCount;
    Random rn = new Random();

    // GreenCap object constructor
    public GreenCap(List<String[]> energyConsumptions, int popSize, float[] maxConsumption, float[] averageConsumption, List<String[]> rules)
            throws IOException {
        this.population = new Population(popSize, energyConsumptions, maxConsumption, averageConsumption, rules);
        this.generationCount = 0;
    }

    // function to implement allocation of appliances using genetic algorithm
    public static List<String[]> GeneticAlgorithm(List<String[]> energyConsumptions, List<String[]> peakHours,
                                                  float[] maxConsumption, float[] averageConsumption, List<String[]> rules)
            throws IOException {
        ...
        GreenCap greenCap = new GreenCap(energyConsumptions, popSize, maxConsumption, averageConsumption, rules);

        // calculate how total consumption of a device in a day for all days of the year
        float[][] dailyConsumption = greenCap.calcConsumptions(energyConsumptions);

        //System.out.println("\nGreenCap:");
        System.out.println("Population of " + greenCap.population.getPopSize() + " chromosome(s).");

        //Calculate fitness of each individual
        greenCap.population.calculateFitness();

        // print current generation and fitness
        System.out.println("Generation: " + greenCap.generationCount + " Fittest Score: " +
                greenCap.population.getFittestScore() + " Exported Energy: " +
                greenCap.population.getFittestExported() + " kWh. Imported Energy: " +
                greenCap.population.getFittestImported() + " kWh. Total Production: " +
                greenCap.population.getFittestProduction() + " kWh. Total Consumption: " +
                greenCap.population.getFittestConsumption() + " kWh.");

        // while current generation is less than max number of generations
        while (greenCap.generationCount < maxGeneration) {

            int iteration = 0;

            // increase the current generation
            ++greenCap.generationCount;
            ...
            while (iteration < popSize) {
                ...
                greenCap.firstSelected.fixConsumptions(dailyConsumption, maxConsumption);
                greenCap.secondSelected.fixConsumptions(dailyConsumption, maxConsumption);

                // update fitness values of offspring and clone best offspring
                greenCap.firstSelected.calcFitness();
                greenCap.secondSelected.calcFitness();
                try {
                    if (greenCap.firstSelected.getFitness() < greenCap.secondSelected.getFitness())
                        greenCap.best = (Chromosome) greenCap.firstSelected.clone();
                    else
                        greenCap.best = (Chromosome) greenCap.secondSelected.clone();
                } catch (CloneNotSupportedException e) {
                    e.printStackTrace();
                }

                // reallocate appliances from peak to non-peak hour
                greenCap.peakHoursReallocation(peakHours);

                // add the fittest offspring to population
                greenCap.addFittestOffspring();
            ...
            greenCap.population.getFittest();
        ...
        //System.out.println("Fitness: " + greenCap.population.getFittestScore());

        for (Chromosome c : greenCap.population.getChromosomes())
            c.fixConsumptions(dailyConsumption, maxConsumption);

        greenCap.population.calculateFitness();
        greenCap.population.getFittest();

        return greenCap.population.getChromosomes()[greenCap.population.getFittestIndex()].getGenes();
    ...
    public void selection() {
        ...
        try {
            ...
            firstSelected = (Chromosome) population.getChromosomes()[firstRandom].clone();
            ...
            secondSelected = (Chromosome) population.getChromosomes()[secondRandom].clone();
        } catch (CloneNotSupportedException e) {
            e.printStackTrace();
        }
    ...
    public void crossover() {
        ...
        // save results
        firstSelected = new Chromosome(first);
        secondSelected = new Chromosome(second);
    ...
    public Chromosome getFittestOffspring() {
    ...
    public void addFittestOffspring() {
        ...
        firstSelected.calcFitness();
        secondSelected.calcFitness();
        best.calcFitness();
        ...
        // replace the least fit individual from the fittest offspring
        population.getChromosomes()[leastFittestIndex] = new Chromosome(getFittestOffspring());
    ...
    public void peakHoursReallocation(List<String[]> peakHours) {
        ...
        // save the results
        best = new Chromosome(reallocatedData);
